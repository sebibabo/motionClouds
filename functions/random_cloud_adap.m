function z = random_cloud_adap(envelope, seed, impulse, do_amp)
...
%     - test the effect of randomizing amplitudes too by setting do_amp to True
    ...
    if impulse
        ...
        rng(seed) ;
        % SB: since the motion clouds are symmetrical,
        % the phase has to be symmetric:
        % p(u,v,w) = -p(-u,-v,-w)
        phase = 2 * pi * rand(N_X, N_Y, N_frame) ;
        % quick and dirty hack to force the spectrum be symmetric
        phase =  angle(fftshift(fftn(real(ifftn(ifftshift( exp(1j*phase)  ))))));
